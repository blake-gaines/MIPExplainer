...
init_graph_comp = dataset.get_random_graph(num_nodes=args.num_nodes)

init_graphs = []
for b in range(batch_size):
    if args.init_with_data:
        init_graphs.append(
            dataset.get_random_graph(args.max_class, num_nodes=args.num_nodes)
        )
...
if dataset.node_feature_type == "one-hot":
    ...
    X = invert_utils.get_node_degree_features(
        m, init_graph.num_nodes, dataset.num_node_features, A
    )
...
# emb_distance = inverter.m.addVar(lb=0, ub=emb_diffs.size, name="emb_dist")
# inverter.m.addConstr(emb_distance == gp.quicksum(emb_diffs * emb_diffs))
# # inverter.m.addGenConstrNorm(emb_distance, emb_diffs.tolist(), 2)

# inverter.tracked_vars["Embedding Distance"] = emb_distance

# inverter.model.addConstr(emb_distance <= 0.1, name="embs_close")

# a_diffs = As[0].reshape((-1,)) - As[1].reshape((-1,))
# a_distance = inverter.m.addVar(lb=0, ub=A.size, name="a_dist")
# inverter.m.addConstr(a_distance == gp.quicksum(a_diffs * a_diffs))
...
if args.log:
    wandb.run.name = (
        f"{dataset.GRAPH_CLS[args.max_class].lower()}-{args.budget}-{wandb.run.id[-4:]}"
    )
